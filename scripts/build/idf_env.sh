...
# This is the directory holding idf_runner.py, NOT the project root: source this
# file from the project root so that idf.py inherits the right cwd.
BUILD_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -W)"

idf() {
  "$IDF_PYTHON" "$BUILD_DIR/idf_runner.py" "$@"
}
