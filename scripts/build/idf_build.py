...
1. idf.py only calls main() when MSYSTEM is absent from the environment
   (see scripts/build/idf.py, `if __name__ == '__main__'`). Git Bash always injects
...
2. A ';'-separated PATH passed through Git Bash gets mangled by MSYS path
   conversion: `D:/Espressif/scripts/build/cmake/3.30.2/bin` becomes
...
follows the toolchain if the installation is ever updated.
    ...
    python scripts/build/idf_build.py build
    python scripts/build/idf_build.py -p COM5 flash
    python scripts/build/idf_build.py size
"""
...
IDF_ENV_BAT = Path(__file__).resolve().parent / "idf_env.bat"
