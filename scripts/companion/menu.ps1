...
.DESCRIPTION
    The programs themselves live at their canonical paths (the Python entry points
    under scripts\windows\, the launchers next to this file) so there is exactly
    one copy of each. This menu is the single place they are all reachable from,
    which is the point of the scripts\companion\ folder.
...
function Get-Entries {
    ...
    $add = {
        ...
        # Two kinds of entry: Python entry points are addressed from the repo
        # root (scripts\windows\...), while the .ps1 launchers are siblings of
        # this menu inside scripts\companion\. Resolving both against the repo
        # root would mark every launcher as missing.
        $path = if ($Script -eq '<self>') { $Script }
        elseif ($Script -like '*.py') { Join-Path $script:RepoRoot $Script }
        else { Join-Path $script:CompanionDir $Script }
    ...
    & $add '只读：打印本机额度 JSON（不碰蓝牙）' 'scripts\windows\windows_companion.py' @('--json-only', '-v') ''
    & $add '写入一次额度' 'scripts\windows\windows_companion.py' @('--once', '-v') ''
    & $add '探测额度服务并保持链路（不写入）' 'scripts\windows\windows_companion.py' @('--probe-only', '--hold-seconds', '60', '-v') ''
    & $add '列出 Windows 已知 BLE 设备' 'scripts\windows\ble_devices.py' @() '确认板子在不在'
    & $add '扫描 BLE 设备' 'scripts\windows\ble_scan.py' @() '约 30 秒'
    & $add '主机侧配对记录诊断' 'scripts\windows\ble_host_diag.py' @() '需要板子地址，会自动填入'
    & $add '重新配对（清主机侧残留绑定）' 'scripts\windows\windows_companion.py' @('--repair-pairing', '-v') '会改动本机蓝牙状态'
    & $add '查看蓝牙网卡/板子的省电策略' 'scripts\windows\bt_power_repair.py' @() '管理员可加 --apply'
    & $add '开关蓝牙无线电（治主机栈假死）' 'scripts\windows\bt_radio_toggle.py' @() '所有蓝牙设备会断几秒'
    & $add '查看板子 HID 能力' 'scripts\windows\hid_caps.py' @() '应看到 usagePage=0xFF00'
    & $add '按桌面端分帧直发一条 RPC' 'scripts\windows\hid_rpc.py' @('--method', 'sys.version', '--listen', '5') ''
    & $add '查看桌面端进程与网卡状态' 'scripts\windows\codex_host_state.py' @() ''
...
while ($true) {
    ...
    switch ($entry.Script) {
        ...
        default {
            ...
            if ($entry.Script -like '*windows_companion.py' -and $needsAddress) {
                $address = Get-BoardAddress -Configured $config.DeviceAddress -Python $python
                if (-not $address) {
