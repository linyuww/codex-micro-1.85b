...
if ($address) {
    ...
    if ($summary -match 'service_discovery=Success' -and $summary -match 'quota_characteristic=yes') {
        ...
        $problems.Add('GATT discovery failed -- see debug/porting-log.md 6.11 (Windows GATT cache) and 6.16 (stale bond)')
    }
}
else {
...
if (Test-Path -LiteralPath $hidScript) {
    ...
    if ($hid -match 'no HID interface') {
        Write-Warn2 'the desktop app has no HID node to open (debug/porting-log.md 6.16: stale host-side record)'
    }
}
